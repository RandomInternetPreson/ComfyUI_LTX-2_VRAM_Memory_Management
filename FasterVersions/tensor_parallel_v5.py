...
class SeqParallelBlockWrapper(nn.Module):
    ...
    def __init__(
        ...
        # NOTE: We no longer create SeqParallelAttention/SeqParallelFFN wrapper modules here
        # because they were unused and adding them as submodules affects ComfyUI's memory management.
        # Instead, we cache module copies directly in _setup_cached_modules().
        
        # Store scale/shift tables for ada operations
        self._setup_ada_params()
        
        # Cache module copies per GPU to avoid .to() calls during forward
        self._cached_modules = {}
        self._setup_cached_modules()
    ...
    def _setup_cached_modules(self):
        """Pre-cache module copies on each GPU to avoid .to() calls during forward."""
        import copy
        ...
        for gpu_id in self.gpu_ids:
            ...
            # Cache attention1 components
            if hasattr(self.original_block, 'attn1'):
                attn = self.original_block.attn1
                cache['attn1'] = {
                    'to_q': copy.deepcopy(attn.to_q).to(device),
                    'to_k': copy.deepcopy(attn.to_k).to(device),
                    'to_v': copy.deepcopy(attn.to_v).to(device),
                    'to_out': copy.deepcopy(attn.to_out).to(device),
                    'q_norm': copy.deepcopy(attn.q_norm).to(device) if attn.q_norm is not None else None,
                    'k_norm': copy.deepcopy(attn.k_norm).to(device) if attn.k_norm is not None else None,
                    'heads': attn.heads,
                    'dim_head': attn.dim_head,
                }
            
            # Cache attention2 components (cross-attention)
            if hasattr(self.original_block, 'attn2'):
                attn2 = self.original_block.attn2
                cache['attn2'] = copy.deepcopy(attn2).to(device)
            
            # Cache FFN
            if hasattr(self.original_block, 'ff'):
                cache['ff'] = copy.deepcopy(self.original_block.ff).to(device)
            
            # Cache AV-specific modules
            if self.is_av_block:
                if hasattr(self.original_block, 'audio_attn1'):
                    cache['audio_attn1'] = copy.deepcopy(self.original_block.audio_attn1).to(device)
                if hasattr(self.original_block, 'audio_attn2'):
                    cache['audio_attn2'] = copy.deepcopy(self.original_block.audio_attn2).to(device)
                if hasattr(self.original_block, 'audio_ff'):
                    cache['audio_ff'] = copy.deepcopy(self.original_block.audio_ff).to(device)
                if hasattr(self.original_block, 'audio_to_video_attn'):
                    cache['audio_to_video_attn'] = copy.deepcopy(self.original_block.audio_to_video_attn).to(device)
                if hasattr(self.original_block, 'video_to_audio_attn'):
                    cache['video_to_audio_attn'] = copy.deepcopy(self.original_block.video_to_audio_attn).to(device)
                # Cache scale_shift tables
                if hasattr(self.original_block, 'scale_shift_table'):
                    cache['scale_shift_table'] = self.original_block.scale_shift_table.to(device)
                if hasattr(self.original_block, 'audio_scale_shift_table'):
                    cache['audio_scale_shift_table'] = self.original_block.audio_scale_shift_table.to(device)
                if hasattr(self.original_block, 'scale_shift_table_a2v_ca_audio'):
                    cache['scale_shift_table_a2v_ca_audio'] = self.original_block.scale_shift_table_a2v_ca_audio.to(device)
                if hasattr(self.original_block, 'scale_shift_table_a2v_ca_video'):
                    cache['scale_shift_table_a2v_ca_video'] = self.original_block.scale_shift_table_a2v_ca_video.to(device)
            
            self._cached_modules[gpu_id] = cache
    ...
    def _single_stream_forward(self, x: torch.Tensor, **kwargs) -> torch.Tensor:
        ...
        for gpu_id in self.gpu_ids:
            ...
            with torch.cuda.device(gpu_id):
                ...
                # Use cached attention weights (pre-copied to this GPU)
                attn_cache = self._cached_modules[gpu_id]['attn1']
                
                q = attn_cache['to_q'](norm_x)
                k = attn_cache['to_k'](norm_x)
                v = attn_cache['to_v'](norm_x)
                
                # Apply Q/K norms if present
                if attn_cache['q_norm'] is not None:
                    q = attn_cache['q_norm'](q)
                if attn_cache['k_norm'] is not None:
                    k = attn_cache['k_norm'](k)
                
                # Reshape for attention FIRST (before RoPE)
                heads = attn_cache['heads']
                dim_head = attn_cache['dim_head']
        ...
        for gpu_id in self.gpu_ids:
            ...
            with torch.cuda.device(gpu_id):
                ...
                # Output projection (use cached module)
                to_out = self._cached_modules[gpu_id]['attn1']['to_out']
        ...
        if hasattr(self.original_block, 'attn2') and v_context is not None:
            for gpu_id in self.gpu_ids:
                ...
                with torch.cuda.device(gpu_id):
                    ...
                    # Use cached attn2 module
                    attn2 = self._cached_modules[gpu_id]['attn2']
        ...
        if hasattr(self.original_block, 'ff'):
            for gpu_id in self.gpu_ids:
                ...
                with torch.cuda.device(gpu_id):
                    ...
                    # Use cached FFN module
                    ff = self._cached_modules[gpu_id]['ff']
    ...
    def _av_forward(self, vx: torch.Tensor, ax: torch.Tensor, **kwargs) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if run_vx and hasattr(self.original_block, 'attn1'):
            ...
            for gpu_id in self.gpu_ids:
                ...
                with torch.cuda.device(compute_device):
                    ...
                    # Get ada values (use cached scale_shift_table)
                    if hasattr(self.original_block, 'scale_shift_table') and v_timestep is not None:
                        ts = v_timestep.to(compute_device)
                        table = self._cached_modules[gpu_id].get('scale_shift_table')
                        if table is None:
                    ...
                    # Compute Q, K, V using cached modules (no auto-loading needed)
                    attn_cache = self._cached_modules[gpu_id]['attn1']
                    q = attn_cache['to_q'](norm_vx)
                    k = attn_cache['to_k'](norm_vx)
                    v = attn_cache['to_v'](norm_vx)
                    del norm_vx
            ...
            for gpu_id in self.gpu_ids:
                ...
                with torch.cuda.device(gpu_id):
                    ...
                    # Output projection using cached module
                    out = self._cached_modules[gpu_id]['attn1']['to_out'](out)
        ...
        if run_ax and hasattr(self.original_block, 'audio_attn1'):
            # Use first chunk GPU for audio (audio is small, doesn't need distribution)
            audio_gpu = self.gpu_ids[0]
            weight_device = f'cuda:{audio_gpu}'
            ...
            with torch.cuda.device(weight_device):
                ...
                # Get ada values for audio (use cached table)
                if hasattr(self.original_block, 'audio_scale_shift_table') and a_timestep is not None:
                    ts = a_timestep.to(weight_device)
                    table = self._cached_modules[audio_gpu].get('audio_scale_shift_table')
                    if table is None:
                        table = self.original_block.audio_scale_shift_table.to(weight_device)
                    ada = self.original_block.get_ada_values(table, batch, ts, slice(0, 3))
                ...
                # Use cached audio_attn1
                audio_attn1 = self._cached_modules[audio_gpu].get('audio_attn1')
                if audio_attn1 is None:
        ...
        if run_ax and hasattr(self.original_block, 'audio_attn2') and a_context is not None:
            ...
            with torch.cuda.device(weight_device):
                ...
                # Use cached audio_attn2
                audio_attn2 = self._cached_modules[audio_gpu].get('audio_attn2')
                if audio_attn2 is None:
        ...
        if run_a2v or run_v2a:
            ...
            with torch.cuda.device(weight_device):
                ...
                if hasattr(self.original_block, 'scale_shift_table_a2v_ca_audio') and a_cross_scale_shift_timestep is not None:
                    ...
                    crossmodal_gpu = self.gpu_ids[crossmodal_gpu_idx]
                    table = self._cached_modules[crossmodal_gpu].get('scale_shift_table_a2v_ca_audio')
                    if table is None:
                        table = self.original_block.scale_shift_table_a2v_ca_audio.to(weight_device)
                ...
                if hasattr(self.original_block, 'scale_shift_table_a2v_ca_video') and v_cross_scale_shift_timestep is not None:
                    crossmodal_gpu = self.gpu_ids[crossmodal_gpu_idx]
                    table = self._cached_modules[crossmodal_gpu].get('scale_shift_table_a2v_ca_video')
                    if table is None:
                        table = self.original_block.scale_shift_table_a2v_ca_video.to(weight_device)
        ...
        if run_a2v and hasattr(self.original_block, 'audio_to_video_attn'):
            crossmodal_gpu = self.gpu_ids[crossmodal_gpu_idx]
            a2v_attn = self._cached_modules[crossmodal_gpu].get('audio_to_video_attn')
            if a2v_attn is None:
        ...
        if run_v2a and hasattr(self.original_block, 'video_to_audio_attn'):
            crossmodal_gpu = self.gpu_ids[crossmodal_gpu_idx]
            v2a_attn = self._cached_modules[crossmodal_gpu].get('video_to_audio_attn')
            if v2a_attn is None:
        ...
        if run_vx and hasattr(self.original_block, 'ff'):
            for gpu_id in self.gpu_ids:
                ...
                with torch.cuda.device(gpu_id):
                    ...
                    # Sub-chunk FFN for memory (use cached ff module)
                    ff = self._cached_modules[gpu_id]['ff']
        ...
        if run_ax and hasattr(self.original_block, 'audio_ff'):
            ...
            with torch.cuda.device(weight_device):
                ...
                # Get ada values (use cached table)
                if hasattr(self.original_block, 'audio_scale_shift_table') and a_timestep is not None:
                    ts = a_timestep.to(weight_device)
                    table = self._cached_modules[audio_gpu].get('audio_scale_shift_table')
                    if table is None:
                ...
                # Use cached audio_ff
                audio_ff = self._cached_modules[audio_gpu].get('audio_ff')
                if audio_ff is None:
